...
public class DitherProcessor {
	...
	private static BufferedImage noneDither(BufferedImage im) {
		int w = im.getWidth(), h = im.getHeight();
		BufferedImage ret = new BufferedImage(w, h,
				BufferedImage.TYPE_BYTE_GRAY);
		for(int i = 0; i < w; i++) {
			for(int j = 0; j < h; j++) {
				int gray = (im.getRGB(i, j) & 0xFF);
				ret.setRGB(i, j, gray > 127 ? 0xFFFFFFFF : 0x0);
			}
		}
		return ret;
	}
	...
	private static final List<DitherMethod> DITHER_METHODS = Arrays.asList(new DitherMethod() {
		@Override
		public String name() {
			return "Simple Binarization";
		}
		@Override
		public BufferedImage process(BufferedImage img) {
			return noneDither(img);
		}
